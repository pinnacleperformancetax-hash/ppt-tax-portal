...
def my_year_end():
    ...
    return render_template_string("""{%extends"base.html"%}{%block content%}<h1>Year-End Summary</h1><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><div style="display:flex;gap:8px"><form method="GET"><select name="year" onchange="this.form.submit()" style="padding:8px 12px;border-radius:10px;border:1px solid #cbd5d1"><option value="2023"{%if year=="2023"%} selected{%endif%}>2023</option><option value="2024"{%if year=="2024"%} selected{%endif%}>2024</option><option value="2025"{%if year=="2025"%} selected{%endif%}>2025</option><option value="2026"{%if year=="2026"%} selected{%endif%}>2026</option></select></form><a href="/my/pl-report?year={{year}}" target="_blank" class="btn" style="padding:8px 14px;font-size:13px;background:#e8f5ec;color:#0b5f2a">Download P&L PDF</a></div></div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:20px"><div class="metric"><span>Total Income</span><strong style="color:#0b5f2a">${{"%.2f"|format(income|float)}}</strong></div><div class="metric"><span>Total Expenses</span><strong style="color:#b91c1c">${{"%.2f"|format(expenses|float)}}</strong></div><div class="metric"><span>Net Profit</span><strong style="color:{{"#0b5f2a"if profit>=0 else"#b91c1c"}}">${{"%.2f"|format(profit|float)}}</strong></div></div>{%if by_category%}<div class="card"><h2 style="margin-top:0">Income & Expenses by Category</h2><div class="table-wrap"><table><thead><tr><th>Category</th><th>Type</th><th style="text-align:right">Total</th></tr></thead><tbody>{%for c in by_category%}<tr><td>{{c.category}}</td><td><span class="pill{%if c.type!="income"%} warn{%endif%}">{{c.type|title}}</span></td><td style="text-align:right;font-weight:900">${{"%.2f"|format(c.total|float)}}</td></tr>{%endfor%}</tbody></table></div></div>{%endif%}{%if documents%}<div class="card"><h2 style="margin-top:0">Documents — {{year}}</h2><div class="table-wrap"><table><thead><tr><th>Document</th><th>Category</th><th>Date</th><th></th></tr></thead><tbody>{%for d in documents%}<tr><td><strong>{{d.display_name}}</strong></td><td style="font-size:12px">{{d.category or"--"}}</td><td style="font-size:12px;color:#475569">{{d.created_at[:10]if d.created_at else"--"}}</td><td>{%if d.filename%}<a href="/documents/download/{{d.id}}" class="btn" style="padding:4px 8px;font-size:11px">Download</a>{%endif%}</td></tr>{%endfor%}</tbody></table></div></div>{%endif%}{%if returns%}<div class="card"><h2 style="margin-top:0">Tax Returns — {{year}}</h2><div class="table-wrap"><table><thead><tr><th>Year</th><th>Service</th><th>Status</th><th>Invoice</th></tr></thead><tbody>{%for r in returns%}<tr><td><strong>{{r.tax_year}}</strong></td><td style="font-size:12px">{{r.service_type or"--"}}</td><td><span class="pill">{{r.status}}</span></td><td style="font-size:12px">{{r.invoice_number or"--"}}</td></tr>{%endfor%}</tbody></table></div></div>{%endif%}{%if invoices%}<div class="card"><h2 style="margin-top:0">Invoices — {{year}}</h2><div class="table-wrap"><table><thead><tr><th>Invoice</th><th>Amount</th><th>Status</th><th></th></tr></thead><tbody>{%for i in invoices%}<tr><td><strong>{{i.invoice_number or"--"}}</strong></td><td style="font-weight:900">${{"%.2f"|format(i.amount|float)}}</td><td><span class="pill{%if i.status=="Overdue"%} warn{%endif%}">{{i.status}}</span></td><td><a href="/invoice/{{i.id}}/pdf" target="_blank" class="btn" style="padding:4px 8px;font-size:11px">PDF</a></td></tr>{%endfor%}</tbody></table></div></div>{%endif%}{%endblock%}""", year=year, income=income, expenses=expenses, profit=profit, by_category=by_category, transactions=transactions, documents=documents, invoices=invoices, returns=returns)
...
def my_invoices():
    ...
    return render_template_string("""{%extends"base.html"%}{%block content%}<h1>My Invoices</h1><p class="sub">View and pay your invoices.</p>{%if invoices%}<div class="table-wrap"><table><thead><tr><th>Invoice</th><th>Description</th><th>Amount</th><th>Due</th><th>Status</th><th></th></tr></thead><tbody>{%for i in invoices%}<tr><td><strong>{{i.invoice_number or"--"}}</strong></td><td style="font-size:12px">{{i.description or"--"}}</td><td style="font-weight:900">${{"%.2f"|format(i.amount|float)}}</td><td style="font-size:12px">{{i.due_date or"--"}}</td><td><span class="pill{%if i.status=="Overdue"%} warn{%endif%}">{{i.status}}</span></td><td style="display:flex;gap:6px;align-items:center">{%if i.status!="Paid"%}<a href="/invoice/{{i.id}}/pay" class="btn" style="padding:6px 12px;font-size:13px">Pay Now</a>{%else%}<span style="color:#11823b;font-size:13px;font-weight:900">Paid ✓</span>{%endif%}<a href="/invoice/{{i.id}}/pdf" target="_blank" class="btn" style="padding:6px 12px;font-size:13px;background:#f1f5f9;color:#0f172a">PDF</a></td></tr>{%endfor%}</tbody></table></div>{%else%}<div class="card"><p style="color:#475569;text-align:center;padding:20px">No invoices yet.</p></div>{%endif%}{%if payments%}<div class="card" style="margin-top:20px"><h2 style="margin-top:0">Payment History</h2><div class="table-wrap"><table><thead><tr><th>Invoice</th><th>Amount</th><th>Method</th><th>Date</th></tr></thead><tbody>{%for p in payments%}<tr><td>{{p.invoice_number or"--"}}</td><td style="font-weight:900;color:#11823b">${{"%.2f"|format(p.amount|float)}}</td><td style="font-size:12px">{{p.method or"--"}}</td><td style="font-size:12px;color:#475569">{{p.created_at[:10]if p.created_at else"--"}}</td></tr>{%endfor%}</tbody></table></div></div>{%endif%}{%endblock%}""", invoices=invoices, payments=payments)
...
def my_bookkeeping():
    ...
    return render_template_string("""{%extends"base.html"%}{%block content%}<h1>My Bookkeeping</h1><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><div style="display:flex;gap:8px"><form method="GET"><select name="year" onchange="this.form.submit()" style="padding:8px 12px;border-radius:10px;border:1px solid #cbd5d1"><option value="2023"{%if year=="2023"%} selected{%endif%}>2023</option><option value="2024"{%if year=="2024"%} selected{%endif%}>2024</option><option value="2025"{%if year=="2025"%} selected{%endif%}>2025</option><option value="2026"{%if year=="2026"%} selected{%endif%}>2026</option></select></form><a href="/my/pl-report?year={{year}}" target="_blank" class="btn" style="padding:8px 14px;font-size:13px;background:#e8f5ec;color:#0b5f2a">Download P&L</a><a href="/my/year-end?year={{year}}" class="btn" style="padding:8px 14px;font-size:13px;background:#f1f5f9;color:#0f172a">Year-End Summary</a></div></div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:20px"><div class="metric"><span>Income</span><strong style="color:#0b5f2a">${{"%.2f"|format(income|float)}}</strong></div><div class="metric"><span>Expenses</span><strong style="color:#b91c1c">${{"%.2f"|format(expenses|float)}}</strong></div><div class="metric"><span>Net Profit</span><strong style="color:{{"#0b5f2a"if profit>=0 else"#b91c1c"}}">${{"%.2f"|format(profit|float)}}</strong></div></div>{%if categories%}<div class="card"><h2 style="margin-top:0">By Category</h2><div class="table-wrap"><table><thead><tr><th>Category</th><th>Type</th><th style="text-align:right">Total</th></tr></thead><tbody>{%for c in categories%}<tr><td>{{c.category}}</td><td><span class="pill{%if c.type!="income"%} warn{%endif%}">{{c.type|title}}</span></td><td style="text-align:right;font-weight:900">${{"%.2f"|format(c.total|float)}}</td></tr>{%endfor%}</tbody></table></div></div>{%endif%}{%if transactions%}<div class="card"><h2 style="margin-top:0">{{transactions|length}} Transaction{{"s"if transactions|length!=1}} — {{year}}</h2><div class="table-wrap"><table><thead><tr><th>Date</th><th>Description</th><th>Category</th><th>Type</th><th style="text-align:right">Amount</th></tr></thead><tbody>{%for t in transactions%}<tr><td style="font-size:12px">{{t.date}}</td><td>{{t.description}}</td><td style="font-size:12px;color:#475569">{{t.category_name or"--"}}</td><td><span class="pill{%if t.type!="income"%} warn{%endif%}">{{t.type|title}}</span></td><td style="text-align:right;font-weight:900">${{"%.2f"|format(t.amount|float)}}</td></tr>{%endfor%}</tbody></table></div></div>{%else%}<div class="card"><p style="color:#475569;text-align:center;padding:20px">No transactions for {{year}}.</p></div>{%endif%}{%endblock%}""", year=year, transactions=transactions, categories=categories, income=income, expenses=expenses, profit=money(income) - money(expenses))
...
def client_dashboard_v2():
    ...
    try: ensure_mega_tables()
    except: pass
    try: ensure_savings_tables()
    except: pass
    try: ensure_upgrade_tables()
    except: pass
    try: ensure_ai_tables()
    except: pass
    try: ensure_workflow_tables()
    except: pass
    ...
    try:
        ...
        recent_docs = query_db("SELECT *,COALESCE(document_name,name,'Document') display_name FROM documents WHERE client_id=? ORDER BY id DESC LIMIT 5", (cid,))
