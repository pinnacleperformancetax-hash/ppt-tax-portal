...
def init_db():
    ...
    migrations=[
        ...
        ("messages","status","TEXT DEFAULT \'Open\'")
    ]
    ...
    for n,k in cats: db.execute("INSERT OR IGNORE INTO categories(name,kind) VALUES (?,?)",(n,k))
    ...
    if db.execute("SELECT COUNT(*) c FROM clients").fetchone()["c"]==0:
        ...
        inv_id=db.execute("INSERT INTO invoices(client_id,invoice_number,issue_date,due_date,amount,status,description) VALUES (?,?,?,?,?,?,?)",(cid,"PPT-1001","2026-04-01","2026-04-15",375.00,"Sent","Tax prep service")).lastrowid
        db.execute("INSERT INTO tax_returns(client_id,tax_year,service_type,status,due_date,fee,notes,invoice_id) VALUES (?,?,?,?,?,?,?,?)",(cid,"2025","1040 + Schedule C","Waiting on Client","2026-04-15",375.00,"Sample return",inv_id))
    db.commit()

@app.route("/init")
def init_route():
    init_db(); return "INIT COMPLETE - database columns upgraded"
...
def dashboard():
    ...
    counts={"clients":query_db("SELECT COUNT(*) c FROM clients",one=True)["c"],"open_invoices":query_db("SELECT COUNT(*) c FROM invoices WHERE status!='Paid'",one=True)["c"],"documents":query_db("SELECT COUNT(*) c FROM documents",one=True)["c"],"returns":query_db("SELECT COUNT(*) c FROM tax_returns",one=True)["c"],"messages":query_db("SELECT COUNT(*) c FROM messages WHERE status='Open'",one=True)["c"]}
    ...
    return render_template("dashboard.html",income=income,expenses=expenses,balance=income-expenses,unpaid=unpaid,counts=counts,recent_transactions=recent_transactions,recent_documents=recent_documents,open_messages=open_messages)
...
def client_dashboard():
    ...
    if not current_user.client_id:
        ...
        return render_template("client_dashboard.html",client=None,documents=[],invoices=[],payments=[],tax_returns=[],messages=[])
    cid=current_user.client_id
    client=query_db("SELECT * FROM clients WHERE id=?",(cid,),one=True)
    documents=query_db("SELECT *,COALESCE(document_name,name,'Document') display_name FROM documents WHERE client_id=? ORDER BY id DESC LIMIT 100",(cid,))
    invoices=query_db("SELECT * FROM invoices WHERE client_id=? ORDER BY id DESC LIMIT 100",(cid,))
    payments=query_db("SELECT p.*,i.invoice_number FROM payments p LEFT JOIN invoices i ON i.id=p.invoice_id WHERE p.client_id=? ORDER BY p.id DESC LIMIT 100",(cid,))
    tax_returns=query_db("SELECT tr.*,i.invoice_number,i.status invoice_status FROM tax_returns tr LEFT JOIN invoices i ON i.id=tr.invoice_id WHERE tr.client_id=? ORDER BY tr.id DESC LIMIT 100",(cid,))
    messages=query_db("SELECT * FROM messages WHERE client_id=? ORDER BY id DESC LIMIT 50",(cid,))
    return render_template("client_dashboard.html",client=client,documents=documents,invoices=invoices,payments=payments,tax_returns=tax_returns,messages=messages)
...
@login_required
def client_message():
    if current_user.role=="admin": return redirect(url_for("messages"))
...
def crm():
    ...
    return render_template("crm.html",leads=query_db("SELECT * FROM crm_leads ORDER BY id DESC LIMIT 500"))
...
def messages():
    if request.method=="POST":
        ...
        flash("Message added to client portal.","success"); return redirect(url_for("messages"))
...
def close_message(message_id):
    ...
    flash("Message closed.","success"); return redirect(url_for("messages"))
...
def settings():
    if request.method=="POST":
        email=request.form.get("email","").lower().strip(); password_hash=generate_password_hash(request.form.get("password") or "Temp123!")
...
def where_for_transactions(alias="t"):
    cid,year,month=report_filters(); cond=[]; args=[]
...
@admin_required
def reports():
    cid,year,month=report_filters(); where,args=where_for_transactions("t")
...
def export_documents():
    ...
    output=io.StringIO(); w=csv.writer(output); w.writerow(["Client","Document","Category","Tax Year","Status","Uploaded"])
    for r in rows: w.writerow([r["client"],r["document"],r["category"],r["tax_year"],r["status"],r["created_at"]])
