...
# These ensure pages work even if template files are missing
# from the templates/ folder on the server.
try:
    from jinja2 import DictLoader, ChoiceLoader as _CL
    _IT = {}
    _IT["service_entry.html"] = """{%extends"base.html"%}{%block content%}<h1>Quick Service Entry</h1><p class="sub">Select one service and fill in only what you need.</p><style>.svc-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:4px}.svc-btn{border:2px solid var(--line);border-radius:16px;padding:18px 10px;text-align:center;cursor:pointer;background:#fff;transition:all .15s}.svc-btn:hover,.svc-btn.active{border-color:var(--g);background:#e8f5ec}.svc-btn .ico{font-size:26px;margin-bottom:5px}.svc-btn .lbl{font-size:12px;font-weight:900;color:#374151}.panel{display:none}.panel.active{display:block}</style><div class="card"><h2 style="margin-bottom:14px">Select a Service</h2><div class="svc-grid"><div class="svc-btn" onclick="pick('transaction',this)"><div class="ico">💰</div><div class="lbl">Transaction</div></div><div class="svc-btn" onclick="pick('invoice',this)"><div class="ico">🧾</div><div class="lbl">Invoice</div></div><div class="svc-btn" onclick="pick('payment',this)"><div class="ico">💳</div><div class="lbl">Payment</div></div><div class="svc-btn" onclick="pick('appointment',this)"><div class="ico">📅</div><div class="lbl">Appointment</div></div><div class="svc-btn" onclick="pick('tax_return',this)"><div class="ico">📁</div><div class="lbl">Tax Return</div></div><div class="svc-btn" onclick="pick('document',this)"><div class="ico">📂</div><div class="lbl">Document</div></div><div class="svc-btn" onclick="pick('message',this)"><div class="ico">✉️</div><div class="lbl">Message</div></div><div class="svc-btn" onclick="pick('crm_lead',this)"><div class="ico">📋</div><div class="lbl">CRM Lead</div></div></div></div><form method="POST" enctype="multipart/form-data"><input type="hidden" name="service_type" id="svc_field"><div class="card panel" id="block-client"><label>Client</label><select name="client_id"><option value="">— No client / Walk-in —</option>{%for c in clients%}<option value="{{c.id}}">{{c.name}}{%if c.business_name%} — {{c.business_name}}{%endif%}</option>{%endfor%}</select></div><div class="card panel" id="panel-transaction"><h2>💰 Transaction</h2><div class="grid grid-3"><div><label>Date</label><input type="date" name="date" value="{{today}}"></div><div><label>Amount</label><input type="number" name="amount" step="0.01" placeholder="0.00"></div><div><label>Type</label><select name="ttype"><option value="income">Income</option><option value="expense">Expense</option></select></div><div style="grid-column:span 2"><label>Description</label><input type="text" name="description" placeholder="e.g. QuickBooks subscription"></div><div><label>Category</label><select name="category_id"><option value="">— Uncategorized —</option>{%for cat in categories%}<option value="{{cat.id}}">[{{cat.kind|title}}] {{cat.name}}</option>{%endfor%}</select></div><div style="grid-column:span 3"><label>Notes</label><textarea name="notes"></textarea></div><div><button type="submit">Save Transaction</button></div></div></div><div class="card panel" id="panel-invoice"><h2>🧾 Invoice</h2><div class="grid grid-3"><div><label>Invoice #</label><input type="text" name="invoice_number" placeholder="Auto if blank"></div><div><label>Amount</label><input type="number" name="amount" step="0.01" placeholder="0.00"></div><div><label>Status</label><select name="status"><option value="Draft">Draft</option><option value="Sent" selected>Sent</option><option value="Paid">Paid</option></select></div><div><label>Issue Date</label><input type="date" name="issue_date" value="{{today}}"></div><div><label>Due Date</label><input type="date" name="due_date"></div><div></div><div style="grid-column:span 3"><label>Description</label><input type="text" name="description" placeholder="e.g. Tax preparation services 2024"></div><div><button type="submit">Save Invoice</button></div></div></div><div class="card panel" id="panel-payment"><h2>💳 Payment</h2><div class="grid grid-3"><div style="grid-column:span 3"><label>Apply to Invoice (optional)</label><select name="invoice_id"><option value="">— Standalone payment —</option>{%for inv in open_invoices%}<option value="{{inv.id}}">{{inv.invoice_number}} — {{inv.client_name}} — ${{"%.2f"|format(inv.amount|float)}}</option>{%endfor%}</select></div><div><label>Amount</label><input type="number" name="amount" step="0.01" placeholder="0.00"></div><div><label>Method</label><select name="method"><option>Manual Entry</option><option>Cash</option><option>Check</option><option>Credit Card</option><option>ACH / Bank Transfer</option><option>Zelle</option><option>PayPal</option></select></div><div><label>Reference #</label><input type="text" name="reference"></div><div style="grid-column:span 3"><label>Notes</label><textarea name="notes"></textarea></div><div><button type="submit">Save Payment</button></div></div></div><div class="card panel" id="panel-appointment"><h2>📅 Appointment</h2><div class="grid grid-3"><div style="grid-column:span 3"><label>Title</label><input type="text" name="title" placeholder="e.g. Tax Review"></div><div><label>Start</label><input type="datetime-local" name="start_at"></div><div><label>End</label><input type="datetime-local" name="end_at"></div><div><label>Status</label><select name="status"><option>Scheduled</option><option>Confirmed</option><option>Completed</option><option>Cancelled</option></select></div><div><label>Location</label><input type="text" name="location"></div><div style="grid-column:span 2"><label>Meeting Link</label><input type="text" name="meeting_link"></div><div style="grid-column:span 3"><label>Notes</label><textarea name="notes"></textarea></div><div><button type="submit">Save Appointment</button></div></div></div><div class="card panel" id="panel-tax_return"><h2>📁 Tax Return</h2><div class="grid grid-3"><div><label>Tax Year</label><input type="text" name="tax_year" value="{{current_year}}"></div><div><label>Service Type</label><select name="service_type_tr"><option value="Individual">Individual (1040)</option><option value="Business">Business</option><option value="S-Corp">S-Corp</option><option value="Partnership">Partnership</option></select></div><div><label>Status</label><select name="status"><option value="In Progress">In Progress</option><option value="Waiting on Client">Waiting on Client</option><option value="Under Review">Under Review</option><option value="Completed">Completed</option><option value="Filed">Filed</option></select></div><div><label>Fee</label><input type="number" name="fee" step="0.01" placeholder="0.00"></div><div><label>Due Date</label><input type="date" name="due_date"></div><div></div><div style="grid-column:span 3"><label>Notes</label><textarea name="notes"></textarea></div><div><button type="submit">Save Tax Return</button></div></div></div><div class="card panel" id="panel-document"><h2>📂 Document</h2><div class="grid grid-3"><div style="grid-column:span 3"><label>File</label><input type="file" name="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.csv,.txt"></div><div style="grid-column:span 2"><label>Document Name</label><input type="text" name="document_name" placeholder="Auto from filename"></div><div><label>Tax Year</label><input type="text" name="tax_year" placeholder="{{current_year}}"></div><div><label>Category</label><select name="category"><option>Tax Documents</option><option>Identification</option><option>Payroll</option><option>Bank Statements</option><option>Receipts</option><option>Other</option></select></div><div style="display:flex;align-items:center;gap:8px;padding-top:28px"><input type="checkbox" name="visible_to_client" value="1" checked style="width:auto;margin:0"><label style="margin:0;font-size:13px">Visible to client</label></div><div></div><div style="grid-column:span 3"><label>Notes</label><textarea name="notes"></textarea></div><div><button type="submit">Save Document</button></div></div></div><div class="card panel" id="panel-message"><h2>✉️ Message</h2><div class="grid"><div><label>Subject</label><input type="text" name="subject" placeholder="Message from Pinnacle Performance Tax"></div><div><label>Message</label><textarea name="body" style="min-height:130px"></textarea></div><div><button type="submit">Send Message</button></div></div></div><div class="card panel" id="panel-crm_lead"><h2>📋 CRM Lead</h2><div class="grid grid-3"><div><label>Name</label><input type="text" name="lead_name" placeholder="Full name"></div><div><label>Phone</label><input type="tel" name="lead_phone"></div><div><label>Email</label><input type="email" name="lead_email"></div><div><label>Source</label><input type="text" name="lead_source" placeholder="Referral, Google..."></div><div><label>Status</label><select name="lead_status"><option>New</option><option>Contacted</option><option>Qualified</option><option>Converted</option><option>Lost</option></select></div><div><label>Follow-up Date</label><input type="date" name="follow_up_date"></div><div style="grid-column:span 3"><label>Notes</label><textarea name="notes"></textarea></div><div><button type="submit">Save Lead</button></div></div></div></form><script>function pick(svc,el){document.querySelectorAll('.svc-btn').forEach(b=>b.classList.remove('active'));el.classList.add('active');document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));document.getElementById('panel-'+svc).classList.add('active');document.getElementById('block-client').classList.add('active');document.getElementById('svc_field').value=svc;document.getElementById('panel-'+svc).scrollIntoView({behavior:'smooth',block:'start'});}</script>{%endblock%}"""
    _IT["workflow_dashboard.html"] = """{%extends"base.html"%}{%block content%}<h1>Workflow Hub</h1><p class="sub">Missing docs, open invoices, appointment requests, open returns.</p><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px;margin-bottom:20px"><div class="metric"><span>Missing Docs</span><strong style="color:#ef4444">{{missing_docs|length}}</strong></div><div class="metric"><span>Open Invoices</span><strong style="color:#f59e0b">{{open_invoices|length}}</strong></div><div class="metric"><span>Appt Requests</span><strong style="color:#0891b2">{{requested_appointments|length}}</strong></div><div class="metric"><span>Open Returns</span><strong style="color:#11823b">{{open_returns|length}}</strong></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:20px"><div class="card"><h2 style="margin-top:0">Missing Documents</h2>{%if missing_docs%}<div class="table-wrap"><table><thead><tr><th>Client</th><th>Document</th><th>Due</th><th></th></tr></thead><tbody>{%for d in missing_docs%}<tr><td>{{d.client_name or"—"}}</td><td><strong>{{d.title}}</strong></td><td style="font-size:12px">{{d.due_date or"—"}}</td><td><form method="POST" action="/document-requests/{{d.id}}/complete"><button style="padding:4px 8px;font-size:11px;background:#e8f5ec;color:#123d22;border:0;border-radius:8px;cursor:pointer">Done</button></form></td></tr>{%endfor%}</tbody></table></div>{%else%}<p style="color:#475569;text-align:center;padding:16px">None ✓</p>{%endif%}<div style="margin-top:12px"><a href="/document-requests" class="btn" style="font-size:12px;padding:8px 12px">+ Request Doc</a></div></div><div class="card"><h2 style="margin-top:0">Open Invoices</h2>{%if open_invoices%}<div class="table-wrap"><table><thead><tr><th>Client</th><th>Invoice</th><th>Amount</th><th>Status</th></tr></thead><tbody>{%for i in open_invoices%}<tr><td>{{i.client_name or"—"}}</td><td style="font-size:12px">{{i.invoice_number or"—"}}</td><td style="font-weight:900">${{"%.2f"|format(i.amount|float)}}</td><td><span class="pill{%if i.status=="Overdue"%} warn{%endif%}">{{i.status}}</span></td></tr>{%endfor%}</tbody></table></div>{%else%}<p style="color:#475569;text-align:center;padding:16px">None ✓</p>{%endif%}</div><div class="card"><h2 style="margin-top:0">Appointment Requests</h2>{%if requested_appointments%}<div class="table-wrap"><table><thead><tr><th>Client</th><th>Title</th><th>When</th><th></th></tr></thead><tbody>{%for a in requested_appointments%}<tr><td>{{a.client_name or"—"}}</td><td>{{a.title or"Appt"}}</td><td style="font-size:12px">{{a.start_at or"—"}}</td><td><form method="POST" action="/appointments/{{a.id}}/approve" style="display:inline"><button style="padding:4px 8px;font-size:11px;background:#e8f5ec;color:#123d22;border:0;border-radius:8px;cursor:pointer">✓</button></form><form method="POST" action="/appointments/{{a.id}}/decline" style="display:inline;margin-left:4px"><button style="padding:4px 8px;font-size:11px;background:#fef2f2;color:#b91c1c;border:0;border-radius:8px;cursor:pointer">✗</button></form></td></tr>{%endfor%}</tbody></table></div>{%else%}<p style="color:#475569;text-align:center;padding:16px">None ✓</p>{%endif%}</div><div class="card"><h2 style="margin-top:0">Tax Returns In Progress</h2>{%if open_returns%}<div class="table-wrap"><table><thead><tr><th>Client</th><th>Year</th><th>Status</th><th>Due</th></tr></thead><tbody>{%for r in open_returns%}<tr><td>{{r.client_name or"—"}}</td><td>{{r.tax_year}}</td><td><span class="pill warn">{{r.status}}</span></td><td style="font-size:12px">{{r.due_date or"—"}}</td></tr>{%endfor%}</tbody></table></div>{%else%}<p style="color:#475569;text-align:center;padding:16px">None ✓</p>{%endif%}</div></div><div class="card"><h2 style="margin-top:0">Quick Actions</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px"><a class="btn" href="/service-entry">⚡ Quick Entry</a><a class="btn" href="/bookkeeping/csv-import">📥 CSV Import</a><a class="btn" href="/bookkeeping/recurring">🔁 Recurring</a><a class="btn" href="/bookkeeping/rules">🏷 Auto Rules</a><a class="btn btn-dark" href="/notifications/list">🔔 Notifications</a></div></div>{%endblock%}"""
    _IT["client_workflow.html"] = """{%extends"base.html"%}{%block content%}<h1>Client Workflow</h1><p class="sub">Track every client through the pipeline.</p><div class="card"><div class="table-wrap"><table><thead><tr><th>Client</th><th>Type</th><th>Workflow</th><th>Priority</th><th>Follow-up</th><th>Actions</th></tr></thead><tbody>{%for c in clients%}<tr><td><strong>{{c.name}}</strong>{%if c.business_name%}<br><span style="font-size:11px;color:#475569">{{c.business_name}}</span>{%endif%}</td><td style="font-size:12px">{{c.client_type or"—"}}</td><td><span class="pill warn">{{c.workflow_status or"Lead"}}</span></td><td style="font-size:12px">{{c.priority or"Standard"}}</td><td style="font-size:12px;color:#475569">{{c.follow_up_date or"—"}}</td><td><form method="POST" action="/clients/{{c.id}}/status" style="display:flex;gap:4px;flex-wrap:wrap"><select name="workflow_status" style="padding:4px 6px;font-size:11px;border-radius:8px">{%for s in["Lead","Consultation","Active","Bookkeeping","Planning","Retention","Inactive"]%}<option value="{{s}}"{%if(c.workflow_status or"Lead")==s%} selected{%endif%}>{{s}}</option>{%endfor%}</select><select name="priority" style="padding:4px 6px;font-size:11px;border-radius:8px">{%for p in["Standard","High","Urgent"]%}<option value="{{p}}"{%if(c.priority or"Standard")==p%} selected{%endif%}>{{p}}</option>{%endfor%}</select><button style="padding:4px 8px;font-size:11px;border-radius:8px">Save</button></form></td></tr>{%else%}<tr><td colspan="6" style="text-align:center;padding:20px;color:#475569">No clients.</td></tr>{%endfor%}</tbody></table></div></div>{%endblock%}"""
    _IT["categorization_rules.html"] = """{%extends"base.html"%}{%block content%}<h1>Auto-Categorization Rules</h1><p class="sub">Keyword rules auto-categorize CSV imports and transactions.</p><div class="card"><h2 style="margin-top:0">Add Rule</h2><form method="POST" class="grid grid-3"><input type="hidden" name="action" value="add"><div><label>Keyword</label><input type="text" name="keyword" placeholder="e.g. QuickBooks" required></div><div><label>Type</label><select name="type"><option value="expense">Expense</option><option value="income">Income</option></select></div><div><label>Category</label><select name="category_id" required><option value="">Select</option>{%for cat in categories%}<option value="{{cat.id}}">[{{cat.kind|title}}] {{cat.name}}</option>{%endfor%}</select></div><div><button type="submit">Add Rule</button></div></form></div><div class="card"><h2 style="margin-top:0">{{rules|length}} Rules</h2>{%if rules%}<div class="table-wrap"><table><thead><tr><th>Keyword</th><th>Type</th><th>Category</th><th>Added</th><th></th></tr></thead><tbody>{%for r in rules%}<tr><td><span class="pill">"{{r.keyword}}"</span></td><td><span class="pill{%if r.type!="income"%} warn{%endif%}">{{r.type|title}}</span></td><td>{{r.category_name or"—"}}</td><td style="font-size:12px;color:#475569">{{r.created_at[:10]if r.created_at else"—"}}</td><td><form method="POST" onsubmit="return confirm('Delete?')"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="{{r.id}}"><button style="padding:5px 10px;font-size:12px;background:#fef2f2;color:#b91c1c;border:0;border-radius:8px;cursor:pointer">Delete</button></form></td></tr>{%endfor%}</tbody></table></div>{%else%}<p style="color:#475569;text-align:center;padding:20px">No rules yet. Add one above — try QuickBooks, Stripe, Google, Gusto.</p>{%endif%}</div>{%endblock%}"""
    _IT["notifications_list.html"] = """{%extends"base.html"%}{%block content%}<h1>Notifications</h1>{%set unread=namespace(count=0)%}{%for n in notifications%}{%if not n.is_read%}{%set unread.count=unread.count+1%}{%endif%}{%endfor%}<p class="sub">{%if unread.count>0%}<span class="pill warn">{{unread.count}} unread</span>{%else%}<span class="pill">All caught up ✓</span>{%endif%}</p><div class="quickbar"><form method="POST" action="/notifications/mark-read"><button type="submit">Mark all read</button></form></div>{%if notifications%}<div class="card" style="padding:0;overflow:hidden">{%for n in notifications%}<div style="display:flex;align-items:flex-start;gap:14px;padding:14px 20px;border-bottom:1px solid #e5e7eb;background:{{'#f0fdf4'if not n.is_read else'#fff'}}"><div style="font-size:22px;padding-top:2px;min-width:28px;text-align:center">{%if n.type=="document"%}📂{%elif n.type=="invoice"%}🧾{%elif n.type=="payment"%}💳{%elif n.type=="message"%}✉️{%elif n.type=="appointment"%}📅{%elif n.type=="bookkeeping"%}💰{%elif n.type=="tax_return"%}📁{%else%}🔔{%endif%}</div><div style="flex:1;min-width:0"><div style="font-size:14px;font-weight:{{'900'if not n.is_read else'400'}};color:#111827">{{n.message}}</div><div style="margin-top:4px;display:flex;align-items:center;gap:10px;flex-wrap:wrap"><span style="font-size:11px;color:#475569">{{n.created_at[:16]if n.created_at else""}}</span>{%if not n.is_read%}<span class="pill">New</span>{%endif%}</div></div><div style="display:flex;gap:6px;align-items:center;flex-shrink:0">{%if n.link%}<a href="{{n.link}}" class="btn" style="padding:6px 12px;font-size:12px">View</a>{%endif%}{%if not n.is_read%}<form method="POST" action="/notifications/mark-read"><input type="hidden" name="id" value="{{n.id}}"><button style="padding:5px 10px;font-size:11px;background:#f1f5f9;color:#475569;border:0;border-radius:8px;cursor:pointer">Mark read</button></form>{%endif%}</div></div>{%endfor%}</div>{%else%}<div class="card"><p style="color:#475569;text-align:center;padding:30px">No notifications yet.</p></div>{%endif%}{%endblock%}"""
    _IT["csv_import.html"] = """{%extends"base.html"%}{%block content%}<h1>CSV Bank Import</h1><p class="sub">Import transactions from a bank export, QuickBooks, or Wave CSV.</p><div class="card"><h2 style="margin-top:0">Import File</h2><form method="POST" enctype="multipart/form-data" class="grid grid-3"><div><label>Client</label><select name="client_id" required><option value="">— Select client —</option>{%for c in clients%}<option value="{{c.id}}">{{c.name}}</option>{%endfor%}</select></div><div style="grid-column:span 2"><label>CSV File</label><input type="file" name="csvfile" accept=".csv" required onchange="preview(this)"></div><div style="grid-column:span 3"><button type="submit">Import Transactions</button></div></form></div><div class="card" id="preview-card" style="display:none"><h2>Preview</h2><div class="table-wrap"><table id="preview-table"></table></div></div><div class="card"><h2>Accepted Column Names</h2><div class="table-wrap"><table><thead><tr><th>Field</th><th>Accepted names</th></tr></thead><tbody><tr><td>Date</td><td>Date, date, DATE</td></tr><tr><td>Description</td><td>Description, description, Memo, memo</td></tr><tr><td>Amount</td><td>Amount, amount, Debit, Credit</td></tr><tr><td>Type</td><td>Type, type (income/expense)</td></tr></tbody></table></div><p style="font-size:12px;color:#475569;margin-top:12px">Negative amounts → expense | Positive → income | $ and commas stripped automatically</p></div><div class="card" style="background:#ecfdf5;border-color:#bbf7d0"><h2>Example CSV</h2><pre style="font-size:12px;background:#fff;padding:12px;border-radius:10px;border:1px solid #bbf7d0;overflow:auto">Date,Description,Amount,Type
2024-01-15,QuickBooks Online,-80.00,expense
2024-01-18,Tax Prep Fee,450.00,income</pre></div><script>function preview(input){const file=input.files[0];if(!file)return;const reader=new FileReader();reader.onload=function(e){const lines=e.target.result.split("\n").filter(l=>l.trim()).slice(0,6);const tbl=document.getElementById("preview-table");tbl.innerHTML="";lines.forEach((line,i)=>{const cells=line.split(",").map(c=>c.replace(/^"|"$/g,"").trim());const tr=document.createElement("tr");cells.forEach(c=>{const el=document.createElement(i===0?"th":"td");el.textContent=c;tr.appendChild(el);});tbl.appendChild(tr);});document.getElementById("preview-card").style.display="block";};reader.readAsText(file);}</script>{%endblock%}"""
    _IT["recurring_transactions.html"] = """{%extends"base.html"%}{%block content%}<h1>Recurring Transactions</h1><p class="sub">Auto-post repeating income and expenses on schedule.</p><div class="card"><h2 style="margin-top:0">Add Recurring Transaction</h2><form method="POST" class="grid grid-3"><input type="hidden" name="action" value="add"><div><label>Client</label><select name="client_id"><option value="">— No specific client —</option>{%for c in clients%}<option value="{{c.id}}">{{c.name}}</option>{%endfor%}</select></div><div style="grid-column:span 2"><label>Description</label><input type="text" name="description" placeholder="e.g. QuickBooks Monthly" required></div><div><label>Amount</label><input type="number" name="amount" step="0.01" placeholder="0.00" required></div><div><label>Type</label><select name="type"><option value="income">Income</option><option value="expense">Expense</option></select></div><div><label>Frequency</label><select name="frequency">{%for f in frequencies%}<option value="{{f}}">{{f|title}}</option>{%endfor%}</select></div><div style="grid-column:span 2"><label>Category</label><select name="category_id"><option value="">— Uncategorized —</option>{%for cat in categories%}<option value="{{cat.id}}">[{{cat.kind|title}}] {{cat.name}}</option>{%endfor%}</select></div><div><label>Notes</label><input type="text" name="notes" placeholder="Optional"></div><div style="grid-column:span 3"><button type="submit">Add Recurring</button></div></form></div><div class="card"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px"><h2 style="margin:0">{{rows|length}} Recurring Transactions</h2><form method="POST" action="/bookkeeping/post-due"><button type="submit" class="btn">Post All Due Today</button></form></div>{%if rows%}<div class="table-wrap"><table><thead><tr><th>Description</th><th>Client</th><th>Amount</th><th>Type</th><th>Frequency</th><th>Next Due</th><th>Status</th><th>Actions</th></tr></thead><tbody>{%for r in rows%}<tr style="{%if not r.is_active%}opacity:.5{%endif%}"><td><strong>{{r.description}}</strong>{%if r.notes%}<br><span style="font-size:11px;color:#475569">{{r.notes}}</span>{%endif%}</td><td>{{r.client_name or"—"}}</td><td style="font-weight:900;color:{{'#123d22'if r.type=='income'else'#b91c1c'}}">${{"%.2f"|format(r.amount)}}</td><td><span class="pill{%if r.type!='income'%} warn{%endif%}">{{r.type|title}}</span></td><td><span class="pill">{{r.frequency|title}}</span></td><td style="font-size:12px">{{r.next_due or"—"}}</td><td><span class="pill{%if not r.is_active%} warn{%endif%}">{{"Active"if r.is_active else"Paused"}}</span></td><td><form method="POST" style="display:inline"><input type="hidden" name="action" value="post_now"><input type="hidden" name="id" value="{{r.id}}"><button style="padding:5px 10px;font-size:12px">▶ Post</button></form><form method="POST" style="display:inline;margin-left:4px"><input type="hidden" name="action" value="toggle"><input type="hidden" name="id" value="{{r.id}}"><button style="padding:5px 10px;font-size:12px;background:#f1f5f9;color:#0f172a">{{"⏸"if r.is_active else"▶️"}}</button></form><form method="POST" style="display:inline;margin-left:4px" onsubmit="return confirm('Delete?')"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="{{r.id}}"><button style="padding:5px 10px;font-size:12px;background:#fef2f2;color:#b91c1c">🗑</button></form></td></tr>{%endfor%}</tbody></table></div>{%else%}<p style="color:#475569;text-align:center;padding:20px">No recurring transactions yet.</p>{%endif%}</div>{%endblock%}"""
    app.jinja_loader = _CL([app.jinja_loader, DictLoader(_IT)])
except Exception:
    pass
